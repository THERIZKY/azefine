...
- `src/app/(user)/*` – public pages (landing, blog, store, services, about) + layout
- `src/app/(user)/admin` – admin dashboard (requires `session.user.role === "admin"`)
- `src/components` – UI building blocks, Navbar/Footer, providers
- `src/context/DataContext.tsx` – mock content/services/products/posts
