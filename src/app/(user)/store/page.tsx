...
import { Filter, Search, SlidersHorizontal, Star, MapPin, ChevronDown } from "lucide-react";
...
export default function Store() {
  ...
  const categories = useMemo(() => (
    ["All", ...Array.from(new Set(products.filter((p) => p.status === "published").map((p) => p.category)))]
  ), [products]);
  ...
  const metaForProduct = (id: string): ProductMeta => {
    const cities = ["Jakarta Selatan", "Bandung", "Surabaya", "Tangerang", "Jakarta Pusat"];
  ...
  return (
    <div className="bg-brand-surface dark:bg-darkbg min-h-screen pt-4 pb-20">
      ...
      <div className="max-w-[1200px] mx-auto px-4 flex gap-6 items-start">
        ...
        <aside className="w-64 flex-shrink-0 bg-white dark:bg-cardbg rounded-xl shadow-card border border-slate-200/70 dark:border-slate-800/70 p-5 hidden lg:block h-fit sticky top-40">
          ...
          <div className="mb-6 border-b border-slate-100 dark:border-slate-800 pb-6">
            <h4 className="font-bold text-sm text-slate-800 dark:text-white mb-3">Kategori</h4>
            <div className="space-y-2">
              {categories.map((cat) => (
                <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <span className={`text-sm group-hover:text-brand transition-colors ${selectedCategory === cat ? "font-bold text-slate-800 dark:text-white" : "text-slate-600 dark:text-slate-400"}`}>
                    {cat}
                  </span>
                </label>
              ))}
          ...
          {/* Harga */}
          <div className="mb-6">
            <h4 className="font-bold text-sm text-slate-800 dark:text-white mb-3">Harga</h4>
          ...
          <Button
            onClick={() => { setSelectedCategory("All"); setMinPrice(""); setMaxPrice(""); setSearchTerm(""); }}
            variant="outline"
        ...
        <main className="flex-1 min-h-[60vh]">
          {sortedProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center bg-white dark:bg-cardbg rounded-xl shadow-card border border-slate-200/70 dark:border-slate-800/70 p-12 text-center h-full">
              ...
              />
              <h3 className="font-bold text-lg text-slate-800 dark:text-white">Oops, produk tidak ditemukan</h3>
              <p className="text-slate-500 text-sm mt-2 mb-6">Coba ganti kata kunci atau kurangi filter pencarianmu.</p>
              <Button onClick={() => { setSearchTerm(""); setSelectedCategory("All"); }} className="!py-2">Hapus Filter</Button>
            </div>
          ) : (
                    ...
                    <div className="aspect-square relative bg-slate-100 dark:bg-darkbg">
                      ...
                      {product.stock === 0 && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                          <span className="bg-slate-800 text-white text-xs font-bold px-3 py-1 rounded-full">Stok Habis</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-3 flex flex-col flex-grow">
                      ...
                      <div className="mt-1">
                        <div className="font-extrabold text-base text-slate-900 dark:text-white">
                          {formatRupiah(product.promoPrice || product.regularPrice)}
                        </div>
                        {discount > 0 && (
                          <div className="flex items-center gap-1.5 mt-1">
                            <span className="bg-red-100 text-red-600 text-[10px] font-bold px-1 rounded">{discount}%</span>
                            <span className="text-[10px] text-slate-400 line-through decoration-slate-400">{formatRupiah(product.regularPrice)}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="mt-auto pt-3">
                        <div className="flex items-center gap-1 mb-1 text-slate-500">
                          <MapPin size={12} />
                          <span className="text-[11px] truncate">{meta.city}</span>
                        </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-[80%] max-w-xs bg-white dark:bg-cardbg shadow-2xl p-5 flex flex-col animate-in slide-in-from-right duration-300">
            <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Filter & Sort</h3>
              <button onClick={() => setIsMobileFilterOpen(false)} className="p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Close filters">
            ...
            <div className="overflow-y-auto flex-1 space-y-6">
              ...
              <div>
                <h4 className="font-bold text-sm text-slate-800 dark:text-white mb-3">Urutkan</h4>
                <select
                  value={sortOption}
                  onChange={(e) => setSortOption(e.target.value as SortOption)}
              ...
              <div>
                <h4 className="font-bold text-sm text-slate-800 dark:text-white mb-3">Range Harga</h4>
                <div className="space-y-3">
                  <input
                    type="number"
                    placeholder="Min"
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 mt-auto">
              <Button onClick={() => setIsMobileFilterOpen(false)} className="w-full">Terapkan</Button>
            </div>
          </div>
        </div>
      )}
