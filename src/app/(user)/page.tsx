...
export default function Home() {
    ...
    const variantClasses: Record<SectionStyle["variant"], string> = {
        ...
        brand: "bg-slate-50 dark:bg-brand text-black dark:text-white",
        alternate:
            "bg-slate-50 dark:bg-card border-y border-slate-100 dark:border-slate-800",
    ...
    const SECTION_COMPONENTS: Record<string, ReactNode> = {
        hero: (
            ...
            >
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8  z-10">
                    <div
                        className={`flex flex-col lg:flex-row gap-16 items-center text-left`}
                    >
                        <div
                            className={`lg:w-1/2 ${heroAlign === "center" ? "lg:w-2/3 mx-auto" : ""}`}
                        >
                            ...
                            <div
                                className={`flex flex-col sm:flex-row gap-4 ${heroAlign === "center" ? "justify-center" : ""}`}
                            >
                                <Button
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className={`mb-16 text-center items-center`}>
                        ...
                        <p className="opacity-70 mt-4 text-lg">
                            Dari kendala manual menjadi sistem otomatis.
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-11 gap-8 items-center">
                        <div className="lg:col-span-5 space-y-4">
                            ...
                            {content.problemsList.map((prob, idx) => (
                                <div
                                    key={idx}
                                    className="bg-white dark:bg-card p-5 border-l-4 border-red-500 shadow-sm rounded-r-xl hover:shadow-md transition-shadow"
                                >
                                    <div className="flex items-start gap-4">
                                        <div className="mt-1 text-red-500 bg-red-50 dark:bg-red-900/20 p-1.5 rounded-full">
                                            <XCircle size={16} />
                                        </div>
                        ...
                        <div className="lg:col-span-5 space-y-4">
                            ...
                            {content.solutionsList.map((sol, idx) => (
                                <div
                                    key={idx}
                                    className="bg-brand-gradient p-px rounded-xl shadow-lg shadow-brand/10"
                                >
        ...
        capabilities: (
            <section className={getSectionClasses("capabilities")}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <SectionTitle
                        title="Lingkup Kapabilitas"
                        subtitle="Ekosistem layanan yang saling mendukung."
                        align={capabilitiesAlign}
                    />

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
                        {content.capabilitiesList.map((cap, idx) => {
                            const CapabilityIcon =
                                capabilityIconMap[cap.title] ?? Layers;
                            return (
                                <Card
                                    key={idx}
                                    className="p-8 flex items-start gap-4 group transition-all duration-300 hover:-translate-y-1 relative z-0 hover:z-10 will-change-transform"
                                >
                        ...
                        <Link
                            href="/services"
                            className="flex flex-col items-center justify-center p-8 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-brand hover:bg-brand/5 dark:hover:bg-brand/10 transition-all group relative z-0 hover:z-10 will-change-transform"
                        >
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className={`mb-16 ${getAlignClass("process")}`}>
                        ...
                        <p className="opacity-70 text-lg">
                            Azefine Worker membantu bisnis merancang, membangun,
                            dan menjalankan solusi teknologi secara end-to-end.
                            Tim kami fleksibel, proses kerja jelas, dan
                            komunikasi rapi agar proyek berjalan terarah dan
                            bisa dipelihara jangka panjang.
                        </p>
                    </div>
                    <div className="grid grid-cols-3 gap-5">
                        {[
                            "Tim profesional terkurasi sesuai kebutuhan proyek.",
                            "Proses kerja jelas: scope, timeline, dan reporting rapi",
                            "Solusi siap pakai, mudah dikembangkan, dan sustainable",
                        ].map((i, index: number) => (
                ...
                <div key={idx} className="relative group p-6 rounded-2xl hover:bg-white dark:hover:bg-cardbg hover:shadow-xl transition-all duration-300 border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{desc || 'Deskripsi tahapan kerja.'}</p>
                    ...
                    <div className="grid grid-cols-1 pt-8">
                        <Link
                            href={"/about"}
                            className={`${buttonVariants()} rounded-4xl bg-brand-gradient text-white p-5`}
                        >
                ...
                <div className="grid grid-cols-2 max-w-7xl mx-auto gap-5">
                    ...
                    <div className="max-w-3/4">
                        <h2 className="text-3xl lg:text-4xl font-extrabold mb-4 text-slate-900 dark:text-white">
                            Kenapa Memilih{" "}
                            <span className="text-brand">Azefine?</span>
                        </h2>
                        <p className="text-lg opacity-70">
                            Kami Bukan Sekadar Vendor, Kami Partner Teknologi
                            Anda
                            <br />
                            <br />
                            Azefine Worker hadir sebagai mitra IT yang memahami
                            kebutuhan bisnis lokal.
                            <br />
                            <br /> <br />
                            Kami tidak hanya memberikan solusi teknis, tetapi
                            juga berpikir sebagai bagian dari tim internal Anda
                            untuk memastikan teknologi mendukung pertumbuhan
        ...
        cta: (
            <section
                className={
                    getSectionClasses("cta") + " relative overflow-hidden"
                }
            >
                ...
                <div
                    className={`max-w-4xl mx-auto px-4 relative z-10 ${getAlignClass("cta")} text-white`}
                >
                    <h2 className="text-3xl md:text-5xl font-extrabold mb-8 tracking-tight drop-shadow-md">
                        {content.finalCtaTitle}
                    </h2>
                    <p className="text-xl opacity-90 mb-12 max-w-2xl font-medium leading-relaxed">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 justify-center">
                        <Button
                            variant="white"
                            onClick={() => router.push("/contact")}
                            className="!px-10 !py-4 text-lg shadow-xl hover:scale-105"
                        >
                            Konsultasi Sekarang
                        </Button>
                        <Button
                            onClick={() => router.push("/contact")}
                            className="bg-transparent border-2 border-white text-white hover:bg-white/10 !px-10 !py-4 text-lg"
                        >
                            Hubungi Kami
                        </Button>
                    </div>
